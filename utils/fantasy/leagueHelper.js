...
export function mapSettings(settings) {
    settings.stat_categories = settings
        .stat_categories
        .stats
        .map(s => {
            s.stat.stat_position_types = s
                .stat
                .stat_position_types
                .map(pt => pt.stat_position_type);

            return s.stat;
        });

    settings.roster_positions = settings
        .roster_positions
...
export function mapScoreboard(scoreboard) {
    ...
    matchups = matchups.map(m => {
        if (m.matchup_grades) {
            m.matchup_grades = m
                .matchup_grades
                .map(grade => {
                    return {team_key: grade.matchup_grade.team_key, grade: grade.matchup_grade.grade};
                });
        }
    ...
    return {matchups: matchups, week: scoreboard.week};
}
    ...
    return leagues.map(l => {
        ...
        subresources.forEach((resource, idx) => {
            switch (resource) {
                ...
                case "scoreboard":
                    league.scoreboard = mapScoreboard(l.league[idx + 1].scoreboard[0].matchups);
                    break;
