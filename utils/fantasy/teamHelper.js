...
import {mergeObjects, flattenObject} from './sharedHelper';
...
export function mapRoster(r) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (players[i].player[3]) {
      player.stats = players[i].player[3].player_stats.stats;
    } else if (players[i].player[2]) {
      player.stats = players[i].player[2].player_stats.stats;
    }
...
export function parseCollection(ts, subresources) {
  ...
  return teams.map(t => {
    // this is only here because user games collection is adding an extra null and I
